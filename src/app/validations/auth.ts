...
export const PaymentDetails = Yup.object().shape({
  ...
  bank_transfer$account_no: Yup.string()
    ...
    // bank_transfer$account_type: Yup.string().when(
    //   "paymentType",
    //   (paymentType: string, field: Yup.StringSchema<string | undefined>) =>
    //     paymentType === "Bank Transfer"
    //       ? field.required("No account type provided")
    //       : field
  ...
  bank_transfer$ifsc_code: Yup.string().min(11,"11 digit required").matches(
    ...
    (paymentType: string, field: Yup.StringSchema<string | undefined>) =>
      paymentType === "Bank Transfer"
        ? field.required("IFSC code required")
        : field
  ),
